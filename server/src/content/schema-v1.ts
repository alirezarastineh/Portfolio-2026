...
export const projectSchema = z.object({
  ...
  /**
   * A relative `/media/<file>` path (the site proxies `/media/*` to the API),
   * a legacy `/projects/*.svg` path, or — in snapshots published before the
   * media pipeline — an absolute `https://api…/media/<file>` URL.
   */
  image: z.string(),
